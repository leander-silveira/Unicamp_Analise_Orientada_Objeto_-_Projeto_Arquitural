...
public class ControladorCaixa {
    ...
    public boolean transferirEntreContas(int numOrigem, int senhaOrigem, int numDestino, float valor) {
        ContaCor contaOrigem = dbContas.buscarConta(numOrigem);
        ContaCor contaDestino = dbContas.buscarConta(numDestino);
        ...
        if (!contaOrigem.debitarValor("Transferência para conta " + numDestino, valor, senhaOrigem)) {
            System.out.println("Transferência não realizada: saldo insuficiente ou senha incorreta.");
            return false;
        }

        contaOrigem.cobrarTaxaOperacao();
        contaDestino.creditarValor("Transferência de conta " + numOrigem, valor);
        contaDestino.cobrarTaxaOperacao();
